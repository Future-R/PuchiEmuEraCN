...
namespace MinorShift.Emuera.GameProc.Function
{
	internal sealed partial class FunctionIdentifier
	{
        ...
        private sealed class FADE_Instruction : AbstractInstruction
        {
            public FADE_Instruction()
            {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state, bool translate = false)
            {
                ...
                float alphaDifference = 1.0f;
                InputRequest req = new InputRequest();
                bool isImmediate = false;
                int singleGroupID = -2;

                //グループIDはコンマで区切られている可能性があるので分割する
                string[] groupIds = groupID.Split(',');

                //要素が複数の場合はグループ指定なので後で処理
                if (groupIds.Length == 1)
                {
                    singleGroupID = int.Parse(groupIds[0]);
                }
                ...
                //背景色の色指定。全画面フェードの場合のみ有効。
                //もう少し汎用的にしたいがとりあえず使う色だけ指定
                switch (color)
                {
                    //赤
                    case 1:
                        exm.Console.fadeColor = Color.Red;
                        break;
                    //黒
                    case 2:
                        exm.Console.fadeColor = Color.Black;
                        break;
                    //白
                    case 3:
                        exm.Console.fadeColor = Color.White;
                        break;
                    //指定が不正の場合は背景色
                    default:
                        exm.Console.fadeColor = exm.Console.bgColor;
                        break;
                }
                
                //グループIDの指定が-1なら画面全体に適用する
                if (singleGroupID == -1)
                {
                    exm.Console.fixImgPrint.effectTargetType = GameView.EffectTarget.Screen;
                    //即時実行の場合は指定された透過率に上書きして終了
                    if (isImmediate == true) {
                        exm.Console.screenAlpha = alpha;
                        return;
                    }
                    else
                    {
                        //現在の透過率との差異を取得
                        alphaDifference = alpha - exm.Console.screenAlpha;
                    }
                }
                else {
                    //即時実行の場合は指定された透過率に上書きして終了
                    if (isImmediate == true)
                    {
                        for (int i = 0; i < groupIds.Length; i++) {
                            exm.Console.fixImgPrint.fixFadeAlpha[int.Parse(groupIds[i])] = alpha;
                        }
                        return;
                    }
                    else {
                        exm.Console.fixImgPrint.effectTargetType = GameView.EffectTarget.Group;
                        for (int i = 0; i < groupIds.Length; i++)
                        {
                            //現在の透過率との差異を取得
                            exm.Console.fixImgPrint.effectTarget[int.Parse(groupIds[i])] = true;
                            //複数グループを同時にフェードさせる場合、先頭グループ画像の透過率を基準とする
                            alphaDifference = alpha - exm.Console.fixImgPrint.fixFadeAlpha[int.Parse(groupIds[i])];
                        }
                    }
                }

                //エフェクト中はキー入力、マウス入力を受け付けない
                req.InputType = InputType.Void;
                req.StopMesskip = true;

                //変化にかかる時間からタイマーのインターバルを除算するとハンドラのコール回数が出る
                //差異を回数で割ると一回の増分が出る
                exm.Console.fadeValue = alphaDifference / (req.Timelimit / exm.Console.EffectInterval);
                exm.Console.fadeResultAlpha = alpha;

                exm.Console.WaitFadeInput(req);

            }
        }
        private sealed class SHAKE_Instruction : AbstractInstruction
        {
            public SHAKE_Instruction()
            {
            ...
            public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state, bool translate = false)
            {
                ...
                //TODO:本機能は画面全体を揺らす事はできない
                //(開発中に需要が出てきたら実装する)
                //現在の透過率との差異を取得
                for (int i = 0; i < groupIds.Length; i++)
                {
                    exm.Console.fixImgPrint.effectTarget[int.Parse(groupIds[i])] = true;
                    exm.Console.shakeQt = (int)shakeQt;
                }
                exm.Console.fixImgPrint.effectTargetType = GameView.EffectTarget.Group;
                ...
                exm.Console.shakeCoefficient = effectCoef;

                exm.Console.WaitShakeInput(req);
            }
        }
		//182101 PCDRP-Update:画面エフェクト機能で修正↑--------------------------
