...
using System.Text;      //182101 PCDRP-Update:SHOPだけ文字数が違い過ぎるので個別対応
using System.IO;        //182101 PCDRP-Update:ヒントファイル出力
	...
	internal sealed partial class Process
	{
		...
		void doNormalFunction(InstructionLine func)
		{
			...
			switch (func.FunctionCode)
			{
                ...
                case FunctionCode.OUTPUTHINTS: //182101 PCDRP-Update:ヒントを出力：★
                    try
                    {
                        //ヒントフォルダ内のdatファイルを列挙する
                        DirectoryInfo di = new DirectoryInfo(@"resources\HINT");
                        FileInfo[] files = di.GetFiles("*.dat", SearchOption.AllDirectories);
                        foreach (FileInfo f in files)
                        {
                            //ヒントフォルダ内のdatファイルをtxtに変更してEXEのフォルダにコピー
                            System.IO.File.Copy(f.DirectoryName + "\\" + Path.GetFileNameWithoutExtension(f.Name) + f.Extension, Directory.GetCurrentDirectory() + "\\" + Path.GetFileNameWithoutExtension(f.Name) + ".txt", true);
                        }
                    }
                    // 例外は握りつぶす
                    catch (System.IO.IOException copyError)
                    {
                        console.PrintError("[System Error] ヒントの出力に失敗しました。対処方法は説明書を参照してください)");
                    }
                    break;
                ...
                case FunctionCode.FONTEDGE:
                    ...
                    if (iValue == 1)
                    {
                        PrintEdgeFont.edgeEnabled = true;
                    }
                    else {
                        PrintEdgeFont.edgeEnabled = false;
                    }
                    break;
                //182101 PCDRP-Update:フォント縁取り機能
