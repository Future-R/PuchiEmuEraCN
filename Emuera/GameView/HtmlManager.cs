			...
			while (!st.EOS)
			{
				...
				else//タグ解析
				{
					...
					if (part != null) {
                        ...
                        {
                            //固定表示指定だった場合は従来のリストには追加せず
                            //専用のリストに追加する
                            console.fixImgPrint.addFixImage((ConsoleImagePart)part,
                                                            ((ConsoleImagePart)part).ResourceName);
                        }
                        else
                        {
							cssList.Add(part);
				...
				case "img":
					{
						...
						int width = 0;
						int ypos = 0;
                        int fxpos = -1;			    //182101 PCDRP-Update:画像固定表示機能で修正
                        int fypos = -1;			    //182101 PCDRP-Update:画像固定表示機能で修正
                        bool reverse = false;	    //182101 PCDRP-Update:画像固定表示機能で修正
						...
						while (wc != null && !wc.EOL)
						{
							...
							//182101 PCDRP-Update:画像固定表示機能で修正↓--------------------------
                            else if (word.Code.Equals("fypos", StringComparison.OrdinalIgnoreCase))
                            {
                                if (fypos != -1)
                                    throw new CodeEE("<" + tag + ">タグに" + word.Code + "属性が2度以上指定されています");
                                if (!int.TryParse(attrValue, out fypos))
                                    throw new CodeEE("<" + tag + ">タグのfypos属性の属性値が数値として解釈できません");
                            }
                            else if (word.Code.Equals("fxpos", StringComparison.OrdinalIgnoreCase))
                            {
                                if (fxpos != -1)
                                    throw new CodeEE("<" + tag + ">タグに" + word.Code + "属性が2度以上指定されています");
                                if (!int.TryParse(attrValue, out fxpos))
                                    throw new CodeEE("<" + tag + ">タグのfxpos属性の属性値が数値として解釈できません");
                            }
                            else if (word.Code.Equals("reverse", StringComparison.OrdinalIgnoreCase))
                            {
                                if (reverse)
                                    throw new CodeEE("<" + tag + ">タグに" + word.Code + "属性が2度以上指定されています");
                                if (!bool.TryParse(attrValue, out reverse))
                                    throw new CodeEE("<" + tag + ">タグのreverse属性の属性値がブール値として解釈できません");
                            }
                            else if (word.Code.Equals("group", StringComparison.OrdinalIgnoreCase))
                            {
                                if (group != -1)
                                    throw new CodeEE("<" + tag + ">タグに" + word.Code + "属性が2度以上指定されています");
                                if (!int.TryParse(attrValue, out group))
                                    throw new CodeEE("<" + tag + ">タグのgroup属性の属性値が数値として解釈できません");
                            }
                            else if (word.Code.Equals("alpha", StringComparison.OrdinalIgnoreCase))
                            {
                                if (alpha != -1.0f)
                                    throw new CodeEE("<" + tag + ">タグに" + word.Code + "属性が2度以上指定されています");
						...
						return new ConsoleImagePart(src, srcb, height, width, ypos, fxpos, fypos, reverse, group, alpha, backGroupInfo);		//182101 PCDRP-Update:画像固定表示機能で修正(コンストラクタに引数追加)
					}
