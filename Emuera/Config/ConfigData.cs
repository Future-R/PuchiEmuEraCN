...
namespace MinorShift.Emuera
{
	...
	internal sealed class ConfigData
	{
		...
		private void setDefault()
		{
            ...
            configArray[i++] = new ConfigItem<int>(ConfigCode.FixImgGroupNumber, "画像固定表示のグループ数", 0);        //182101 PCDRP-Update 画像固定表示機能
            configArray[i++] = new ConfigItem<int>(ConfigCode.PrintCShopLength, "SHOPのアイテム時専用のPRINTCの文字数", 30); //182101 PCDRP-Update:SHOPだけ文字数が違い過ぎるので個別対応
...
#region getitem
		...
		public SingleTerm GetConfigValueInERB(string text, ref string errMes)
		{
			...
			switch(item.Code)
			{
                ...
                case ConfigCode.PrintCShopLength:// "PRINTCの文字数(SHOP専用)"        //182101 PCDRP-Update:SHOPだけ文字数が違い過ぎるので個別対応
                ...
                case ConfigCode.EdgeColor://"フォント縁取り色"
                    {
